...
test("keeps editor safety and render checks in the source", async () => {
  ...
  assert.doesNotMatch(page, /zoomEnabled|zoomMarkerEnabled|editor-camera|editor-effects|startMapPointDrag/);
  assert.doesNotMatch(css, /zoom-camera-target|zoom-center-marker|map-zoom-badge|marker-effect-/);
...
test("keeps preview and FFmpeg render settings aligned", async () => {
  ...
  assert.doesNotMatch(page, /playbackMapScale|mapPreviewZoom|transitionDuration: playing/);
  ...
  assert.doesNotMatch(renderer, /zoompan|createZoomMarker|markerEffects|scene\.zoom/);
