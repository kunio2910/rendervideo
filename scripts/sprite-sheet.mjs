...
const detectEmbeddedSpritePanel = (data, info) => {
  ...
  for (let gridY = 0; gridY < gridHeight; gridY += 1) {
    ...
    for (let gridX = 0; gridX < gridWidth; gridX += 1) {
      ...
      neutral[index] = saturation < 28 && light <= 150 ? 1 : 0;
...
const makeSolidForeground = (data, info, background) => {
  ...
  const darkBackground = luminance(background[0], background[1], background[2]) < 28;
  for (let index = 0; index < pixels; index += 1) {
    ...
    const neutralBackgroundLike = saturation < 38 && (darkBackground ? light < 24 : light < 155);
...
const detectRegularGrid = ({ alpha, width, height, alphaThreshold = 24 }) => {
  const active = new Uint8Array(alpha.length);
  const horizontalProjection = new Float32Array(width);
  const verticalProjection = new Float32Array(height);
  for (let index = 0; index < alpha.length; index += 1) {
    if (alpha[index] < alphaThreshold) continue;
    active[index] = 1;
    horizontalProjection[index % width] += 1;
    verticalProjection[Math.floor(index / width)] += 1;
  }
  for (let x = 0; x < width; x += 1) horizontalProjection[x] /= Math.max(1, height);
  for (let y = 0; y < height; y += 1) verticalProjection[y] /= Math.max(1, width);

  const findGaps = (projection, length) => {
    const medianValue = median(Array.from(projection));
    const threshold = Math.max(0.002, Math.min(0.055, medianValue * 0.22 + 0.004));
    const minimumRun = Math.max(2, Math.round(length / 420));
    const gaps = [];
    let start = -1;
    for (let index = 1; index <= length; index += 1) {
      const isGap = index < length - 1 && projection[index] <= threshold;
      if (isGap && start < 0) start = index;
      if (isGap) continue;
      if (start >= 0) {
        const runLength = index - start;
        if (runLength >= minimumRun && start > length * 0.015 && index < length * 0.985) {
          gaps.push({ position: (start + index) / 2, width: runLength });
        }
        start = -1;
      }
    }
  ...
  const xGaps = findGaps(horizontalProjection, width).gaps;
  const yGaps = findGaps(verticalProjection, height).gaps;
  if (xGaps.length < 1 || yGaps.length < 1) return null;

  const integral = new Int32Array((width + 1) * (height + 1));
  for (let y = 0; y < height; y += 1) {
    let rowTotal = 0;
    for (let x = 0; x < width; x += 1) {
      rowTotal += active[y * width + x];
      const index = (y + 1) * (width + 1) + x + 1;
      integral[index] = integral[index - width - 1] + rowTotal;
    }
  }

  const evaluateAxis = (projection, gaps, length, count) => {
    const cellSize = length / count;
    const expected = Array.from({ length: count - 1 }, (_, index) => (index + 1) * cellSize);
    const gapTolerance = Math.max(4, cellSize * 0.2);
    const matched = expected.map((position) => {
      const closest = gaps.reduce((best, gap) => Math.min(best, Math.abs(gap.position - position)), Number.POSITIVE_INFINITY);
      const sampleRadius = Math.max(2, Math.round(cellSize * 0.025));
      const sampleStart = Math.max(0, Math.round(position) - sampleRadius);
      const sampleEnd = Math.min(length, Math.round(position) + sampleRadius + 1);
      let sampleTotal = 0;
      for (let index = sampleStart; index < sampleEnd; index += 1) sampleTotal += projection[index];
      const sampleAverage = sampleTotal / Math.max(1, sampleEnd - sampleStart);
      return {
        gap: closest <= gapTolerance ? 1 - closest / gapTolerance : 0,
        emptiness: 1 - Math.min(1, sampleAverage / 0.32),
      };
    });
    const coverage = matched.length ? matched.reduce((sum, item) => sum + item.gap, 0) / matched.length : 1;
    const emptiness = matched.length ? matched.reduce((sum, item) => sum + item.emptiness, 0) / matched.length : 0;
    const extraGaps = gaps.filter((gap) => expected.every((position) => Math.abs(gap.position - position) > gapTolerance)).length;
    return { cellSize, score: coverage * 0.58 + emptiness * 0.42 - extraGaps * 0.07, coverage, extraGaps };
  };

  let best = null;
  const maxColumns = Math.min(12, Math.floor(width / 32));
  const maxRows = Math.min(12, Math.floor(height / 32));
  for (let columns = 2; columns <= maxColumns; columns += 1) {
    const xEvaluation = evaluateAxis(horizontalProjection, xGaps, width, columns);
    if (xEvaluation.coverage < 0.58) continue;
    for (let rows = 2; rows <= maxRows; rows += 1) {
      const yEvaluation = evaluateAxis(verticalProjection, yGaps, height, rows);
      if (yEvaluation.coverage < 0.58) continue;
      ...
      for (let row = 0; row < rows; row += 1) {
        ...
        for (let column = 0; column < columns; column += 1) {
          ...
          const insetX = Math.max(1, Math.round((right - left) * 0.08));
          const insetY = Math.max(1, Math.round((bottom - top) * 0.08));
          const innerLeft = Math.min(right, left + insetX);
          const innerTop = Math.min(bottom, top + insetY);
          const innerRight = Math.max(innerLeft, right - insetX);
          const innerBottom = Math.max(innerTop, bottom - insetY);
          const innerWidth = innerRight - innerLeft;
          const innerHeight = innerBottom - innerTop;
          const count = integralSum(integral, width, innerLeft, innerTop, innerWidth, innerHeight);
          occupancies.push(count / Math.max(1, innerWidth * innerHeight));
          boxes.push({ left, top, right, bottom, pixelCount: count });
        }
      }
      const sortedOccupancies = [...occupancies].sort((left, right) => left - right);
      const minimumOccupancy = sortedOccupancies[0] ?? 0;
      const medianOccupancy = sortedOccupancies[Math.floor(sortedOccupancies.length / 2)] ?? 0;
      const populated = occupancies.filter((value) => value >= Math.max(0.006, medianOccupancy * 0.22)).length / occupancies.length;
      if (minimumOccupancy < 0.002 || populated < 0.78) continue;
      const score = (xEvaluation.score + yEvaluation.score) / 2
        + Math.min(0.18, medianOccupancy * 0.5)
        + populated * 0.06
        + Math.min(0.04, (columns * rows) / 5000);
  ...
  return { ...best, confidence: Math.round(Math.min(0.99, best.score) * 100) };
};
...
export const processSpriteSheetBuffer = async (input, options = {}) => {
  ...
  if (hasMeaningfulTransparency(pixels, info)) {
    ...
    if (boxes.length < 3) {
      grid = detectRegularGrid({ alpha: foreground.alpha, width: info.width, height: info.height, alphaThreshold: 70 });
      if (grid) {
        boxes = grid.boxes;
  ...
  if (boxes.length < 3) {
    ...
    grid = detectRegularGrid({ alpha: foreground.alpha, width: info.width, height: info.height });
    if (grid) {
      boxes = grid.boxes;
      cellSize = grid.cellSize;
      mode = "regular-grid";
    ...
    if (boxes.length >= 3) {
      // The regular grid is more reliable for contact sheets; use connected
      // components only as a fallback for irregularly placed sprites.
      boxes = grid?.boxes ?? boxes;
    }
    if (boxes.length >= 3) {
      // Skip the component pass when the grid has already identified every cell.
    } else {
    boxes = detectFrameBoxes({
      alpha: foreground.alpha,
      width: info.width,
  ...
  return {
    ...
    ...(grid ? { columns: grid.columns, rows: grid.rows, confidence: grid.confidence } : {}),
    mode,
  };
};
